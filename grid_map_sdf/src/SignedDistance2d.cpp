...
*/
std::vector<DistanceLowerBound>::iterator fillLowerBounds(
 ...
 // Compute bounds to the right of minimum
  ...
  for (Eigen::Index q = start + 1; q < n; ++q) {
   // Storing this by value gives better performance (removed indirection?)
    ...
    // Can ignore the lower bound derived from this point if it is only active outside of [start, n]
    if (s < nFloat) {
      ...
      if (s > startFloat) {
        // Intersection is within [start, n]. Adjust current lowerbound and insert the new one after
        ...
        *rhsBoundIt = DistanceLowerBound{qFloat, fq, s, INF};  // Valid from s till infinity
        ...
        // Intersection is outside [0, n], so the new bound dominates all previous bounds
        *rhsBoundIt = DistanceLowerBound{qFloat, fq, -INF, INF};
        // No need to keep other bounds, so update the first bound iterator to this one.
        firstBoundIt = rhsBoundIt;
      }
    }
...
void extractSquareDistances(
  Eigen::Ref<Eigen::VectorXf> squareDistance1d,
  ...
  Eigen::Index start)
{
 ...
 // Store active bound by value to remove indirection
  ...
  for (Eigen::Index q = start; q < n; ++q) {
    // Observe that if squareDistance1d[q] == 0.0, this is already the minimum and it will stay 0.0
  ...
  for (Eigen::Index q = start; q < n; ++q) {
    ...
    if (squareDistance1d[q] > 0.0F) {
     // Find the new active lower bound if q no longer belongs to current interval
      ...
      squareDistance1d[q] = std::sqrt(squarePixelBorderDistance(qFloat, lowerBoundIt->v,
              lowerBoundIt->f));
    }
...
inline void squaredDistanceTransform_1d_inplace(
  Eigen::Ref<Eigen::VectorXf> squareDistance1d,
...
*/
inline void distanceTransform_1d_inplace(
  Eigen::Ref<Eigen::VectorXf> squareDistance1d,
  std::vector<DistanceLowerBound> & lowerBounds)
{
  auto start = lastZeroFromFront(squareDistance1d);
...
void initializeObstacleDistance(
  const Matrix & elevationMap, Matrix & result, float height,
  float resolution)
{
...
void initializeObstacleFreeDistance(
  const Matrix & elevationMap, Matrix & result, float height,
  float resolution)
{
 /* Vectorized implementation of:
...
void pixelDistanceToFreeSpaceTranspose(
  const Matrix & elevationMap, Matrix & sdfObstacleFree,
  Matrix & tmp, float height, float resolution)
{
...
void pixelDistanceToObstacleTranspose(
  const Matrix & elevationMap, Matrix & sdfObstacleTranspose, Matrix & tmp, float height,
  float resolution)
{
  ...
  float resolution, float minHeight, float maxHeight)
{
  ...
  if (allPixelsAreObstacles) {
    internal::pixelDistanceToFreeSpaceTranspose(elevationMap, sdfTranspose, tmp, height,
          resolution);
    ...
    internal::pixelDistanceToObstacleTranspose(elevationMap, sdfTranspose, tmp, height, resolution);
    internal::pixelDistanceToFreeSpaceTranspose(elevationMap, tmpTranspose, tmp, height,
          resolution);
  ...
  float minHeight, float maxHeight)
{
  ...
  signedDistanceAtHeightTranspose(elevationMap, sdfTranspose, tmp, tmpTranspose, height, resolution,
        minHeight, maxHeight);
